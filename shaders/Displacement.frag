...
void main() {
  // YOUR CODE HERE
      ...
      float dU = (h(vec2(v_uv[0] + 1.0 / float(u_texture_3_size[0]), v_uv[1])) - h(v_uv)) * u_height_scaling * u_normal_scaling;
      float dV = (h(vec2(v_uv[0], v_uv[1] + 1.0 / float(u_texture_3_size[1]))) - h(v_uv)) * u_height_scaling * u_normal_scaling;
